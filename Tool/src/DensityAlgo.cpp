    ...
    for(auto info: reqd.infolist){
        ...
        if(!reqd.condOR[info].empty() && reqd.condOR[info].find(uinfo)==reqd.condOR[info].end()){
            /*if(u.rep.info["lang"] == "en"){
                //cerr << info << " " << u.rep.info[info] << endl;
            }*/
            ret = false;
        }
    }
...
int DensityAlgo::run(map<string, Graph> &pred, InfoSets &reqdPred, map<pair<wordData, wordData>, float> &entries, bool transfull=false)
{
    ...
    {
        /*if(i%50000==0 && i) {
            cerr << i << endl; //output every 1000th node just to check progress
        }*/ //Debugging output
        if(!wordIsReq(G.vertices[i], reqdPred)) continue;
        //cerr << G.vertices[i].rep.surface << " " << num_trans << endl;
        ...
        if(config.transitive == 0){
            ...
            //cerr << G.vertices[i].rep.surface << " " << cyccount << endl;
